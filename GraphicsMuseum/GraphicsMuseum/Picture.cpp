...
Picture::Picture(bool isMoving, vec3 position, vec3 rotation, vec3 scale, vec3 velocity, vec4 color) : 
	GlobalObject(isMoving, position, rotation, scale, velocity, color)
{ } 
...
void Picture::render() {
	if (this->va_vertices.size() == 0) {	
		...
		//The Picture is drawn by specifying individual vertexes. In this case, the top half of the
		//Picture is drawn by loops that draw verticle stacks of circles of non-linear radius which are connected to form
